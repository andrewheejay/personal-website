...
export const metadata: Metadata = {
  title: "lime — Andrew Lee",
  description: "An AI digital wardrobe that tells you what to wear — vision tagging, vector search, and weather-aware outfit recommendations.",
};
    ...
    <div className="text-fg text-body">
      ...
      <header className="space-y-3 pb-8 border-b border-fg/10">
        ...
        <div className="flex items-baseline gap-3 flex-wrap">
          <h1 className="font-bold text-title lowercase text-fg">lime</h1>
          <span className="text-body text-fg">2026</span>
        </div>
        <p className="text-lede text-fg lowercase max-w-measure">an ai digital wardrobe that tells you what to wear</p>
        <p className="text-body text-fg lowercase max-w-measure">
          {["next.js", "fastapi", "gemini", "pinecone", "supabase", "vision-ai", "vector-search"].join("; ")}
        </p>
        <div className="flex gap-4 text-meta text-fg lowercase pt-1">
          <a href="https://github.com/andrewheejay/Lime" target="_blank" rel="noopener noreferrer" className="inline-flex items-center min-h-[24px]">github →</a>
        </div>
      </header>
      ...
      <section className="py-8 border-t border-fg/10 space-y-4">
        ...
        <div className="space-y-4 lowercase text-fg max-w-measure">
          <p>
            the piece i&apos;m proudest of is the vision ingestion pipeline, because it&apos;s where the &quot;no manual data entry&quot; promise either holds up or falls apart. when you snap a photo of an item, a few things happen in sequence on the backend. first, <code className="font-mono text-code text-fg normal-case">briaai/RMBG-1.4</code> strips the background so the model sees the garment and not your bedroom floor. then gemini 2.0 flash tags it across five attributes: category, silhouette, palette, texture, and aesthetic. i forced that output through a pydantic schema so the model has to return structured, validated fields instead of a paragraph i&apos;d have to parse and trust. finally the item gets embedded with <code className="font-mono text-code text-fg normal-case">all-MiniLM-L6-v2</code> into a 384-dimension vector and stored in pinecone for similarity search.
          </p>
          <p>
